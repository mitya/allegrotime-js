...
describe 'Dataset', ->
  ...
  it "is loaded when the app is launched", ->
    expect(appState.crossings.length).toEqual(27)
    expect(appState.closings.length).toEqual(appState.crossings.length * 16)
    expect(appState.crossings[0].name).toEqual('Удельная')
    expect(appState.crossings[0].closings.length).toEqual(16)
